...
@torch.no_grad()
def evaluate_induc(name, model, g, mode, result_file_name=None, dtype=torch.float32):
    """
    mode: 'val' or 'test'
    """
    model.eval()
    feat, labels = g.ndata["feat"].to(dtype=dtype), g.ndata["label"]
...
@torch.no_grad()
def evaluate_trans(name, model, g, result_file_name=None, dtype=torch.float32):
    model.eval()
    feat, labels = g.ndata["feat"].to(dtype=dtype), g.ndata["label"]
...
def move_to_cuda(graph, in_graph, out_graph, node_dict, boundary):
    ...
    for i in range(size):
        if i != rank:
            boundary[i] = boundary[i].to(device)
    for key in node_dict.keys():
        node_dict[key] = node_dict[key].to(device)
    graph = graph.int().to(device)
    in_graph = in_graph.int().to(device)
    out_graph = out_graph.int().to(device)
...
def collect_out_degree(node_dict, boundary):
    ...
    recv_out_deg = data_transfer(
        send_info, recv_shape, tag=TransferTag.DEG, dtype=torch.long
    )
...
def precompute(part, graph, node_dict, boundary, model, gpb, pos):
    ...
    recv_feat = data_transfer(
        send_info, recv_shape, tag=TransferTag.FEAT, dtype=torch.float
    )
    if model == "gcn":
        ...
        with graph.local_scope():
            ...
            graph["_E"].update_all(
                fn.copy_u(u="h", out="m"), fn.sum(msg="m", out="h"), etype="_E"
            )
        ...
        with graph.local_scope():
            graph.nodes["_U"].data["h"] = merge_feature(feat, recv_feat)
            graph["_E"].update_all(
                fn.copy_u(u="h", out="m"), fn.mean(msg="m", out="h"), etype="_E"
            )
...
def select_node(boundary, send_size):
    ...
    for i in range(size):
        ...
        b = boundary[i]
        idx = torch.as_tensor(
            np.random.choice(b.shape[0], send_size[i], replace=False),
            dtype=torch.long,
            device="cuda",
        )
...
def run(graph, node_dict, gpb, args):
    ...
    if args.eval:
        ...
        val_g = val_g.int().to(device)
        test_g = test_g.int().to(device)
    else:
        val_g = test_g = None
    ...
    print(f'collect started')

    node_dict["out_deg"] = collect_out_degree(node_dict, boundary)
    print(f'collect finished')


    if args.use_pp:
        node_dict["feat"] = precompute(
            in_graph, graph, node_dict, boundary, args.model, gpb, pos
        )
    print(f'pp completed')
    ...
    for epoch in range(args.n_epochs):
        ...
        one_hops = data_transfer(
            selected, recv_shape, tag=TransferTag.NODE, dtype=torch.long
        )
        ...
        model.train()
        # with torch.amp.autocast(device_type='cuda', enabled=args.half, dtype=torch.float16):
        if args.model == "gcn":
        ...
        # print(f'logits: {logits}, logits.dtype: {logits.dtype}')
        loss = loss_fcn(logits[train_mask], labels[train_mask])

        pre_reduce, reduce_time = None, None
        ...
        #     print(f"After step Layer: {name}")
        #     print(f"    Weights: {param.data}")
        #     print(f"    Gradient: {param.grad}")
        # for name, param in model.named_parameters():
        #     if param.grad is not None:
        #         print(name, "grad min:", torch.min(param.grad).item(), "grad max:", torch.max(param.grad).item())
    ...
    if args.eval and rank == 0:
        if thread is not None:
            ...
            if val_acc > best_acc:
                ...
                best_model = model_copy
        torch.save(
            best_model.state_dict(), "checkpoint/" + args.graph_name + "_final.pth.tar"
        )
        print("model saved")
